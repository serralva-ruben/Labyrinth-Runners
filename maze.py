...
class MazeGenerator:
    def __init__(self, nr_max_x:int, nr_max_y:int, grid):
        self.width = nr_max_x
    ...
    def frontier(self,x,y):
        n = set()
        if x >= 1 and x < self.width-1 and y >= 0 and y < self.height:
            if x > 1 and self.isWall(self.grid[x-2][y]):
                n.add((x-2, y))
            if x < self.width-3 and self.isWall(self.grid[x+2][y]):
                n.add((x+2, y))
            if y > 1 and self.isWall(self.grid[x][y-2]):
                n.add((x, y-2))
            if y < self.height-3 and self.isWall(self.grid[x][y+2]):
                n.add((x, y+2))
        return n
    ...
    def neighbours(self, x, y):
        ...
        if x >= 0 and x < self.width and y >= 0 and y < self.height:
            if x > 1 and not self.isWall(self.grid[x-2][y]):
                n.add((x-2, y))
            if x < self.width-3 and not self.isWall(self.grid[x+2][y]):
                n.add((x+2, y))
            if y > 1 and not self.isWall(self.grid[x][y-2]):
                n.add((x, y-2))
            if y < self.height-3 and not self.isWall(self.grid[x][y+2]):
                n.add((x, y+2))
        return n

    def isWall(self,cell):
        if(cell==1): return True
        elif(cell==0): return False
        else: return None
